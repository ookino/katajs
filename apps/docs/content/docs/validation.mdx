---
title: 'Validation'
description: 'Zod validation at the route boundary; preserves Hono RPC type inference.'
---
...
The throw propagates through Hono's call stack and lands in [`errorMapper`](/docs/errors), which maps `ValidationError` (an `AppError` subclass) to a 400 response with structured issue data:
