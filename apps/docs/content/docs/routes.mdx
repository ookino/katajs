---
title: 'Routes'
description: 'Mounting Hono routes through createApp while preserving RPC types end-to-end.'
---
...
Every routed module ships a single `<module>.routes.ts` file. `katajs add route` appends to that file; nothing in the CLI or templates supports multiple route files per module by default. This is intentional, the same way [flat module internals are intentional](/docs/modules#module-internal-layout-is-flat).
...
Anything thrown in a route handler — domain errors, validation errors, unexpected exceptions — is caught by `errorMapper` and turned into a structured JSON response. See [Errors](/docs/errors) for the full mapping.
