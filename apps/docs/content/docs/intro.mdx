---
title: 'Intro'
description: 'What katajs is, what it isn''t, and who it''s for.'
---
...
- **[Modules](/docs/modules)** — the unit of organization. What `provides`/`requires` mean and how routed/services-only modules differ.
- **[Container](/docs/container)** — request-scoped DI. How `c.var.resolve(key)` works, what's lazy, what's strict.
- **[Registry](/docs/registry)** — TypeScript module augmentation. How each module's slice composes into the global Registry that powers strict autocomplete.
- **[Routes](/docs/routes)** — how routes mount, where free-floating routes live, how Hono RPC types flow through `createApp`.
- **[Validation](/docs/validation)** — `validate({ body, query, param })`, how Zod errors become structured 400 responses.
- **[Errors](/docs/errors)** — `AppError` subclasses, `errorMapper`, the response shape.
- **[Transactions](/docs/transactions)** — `withTransaction`, repository pattern, cross-module atomicity.
- **[Databases](/docs/databases)** — one database or many; `c.db` as a client or a named map; the `katajs add database` codemod.
- **[Testing](/docs/testing)** — `makeTestContainer` for unit tests, real Postgres for integration tests.
- **[Devtools](/docs/devtools)** — the static module graph (Shape A) and the live interactive devtools UI (Shape B).
- **[Architecture](/docs/architecture)** — when to split modules, when to graduate to `--monorepo`.
