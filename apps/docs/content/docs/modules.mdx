---
title: 'Modules'
description: 'The unit of organization: provides, requires, routed vs services-only.'
---
...
See [Registry](/docs/registry) for the full story on why this pattern.
...
- **Separate entries in the devtools graph.** [`inspectModules()`](/docs/devtools) shows each module as a node with its own routes and dependencies. A split graph is easier to read than one giant node with dozens of services.
...
See [Architecture](/docs/architecture) for splitting heuristics in more depth.
