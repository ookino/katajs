---
title: 'Architecture'
description: 'When to split modules and when to graduate to a --monorepo shape.'
---


This page covers the structural decisions you'll make at the project level: when a module is "too big," when to split into sibling modules, and (later) when to graduate from a single-API project to a `--monorepo` layout. It's the synthesis of the [modules](/docs/modules), [container](/docs/container), and [routes](/docs/routes) pages — the practical "how do I shape this" guide.
...
The [`inspectModules()`](/docs/devtools) graph rewards splitting in a way that's hard to overstate. Compare:
