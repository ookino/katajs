...
## Mixing dialects (Postgres + MySQL)

Each entry in the `db` map is an independent adapter, so dialects are free to mix. `@katajs/drizzle` ships dialect-specific entry points so each one only pulls its own driver into the bundle:

```ts
import { drizzleAdapter } from '@katajs/drizzle';            // Postgres (postgres.js)
import { drizzleMysqlAdapter } from '@katajs/drizzle/mysql'; // MySQL (mysql2)
...
The `AppDb` augmentation then mixes client types — `DrizzleClient` for the Postgres entry, `DrizzleMysqlClient` for the MySQL one. See the [`@katajs/drizzle` README](https://github.com/ookino/katajs/tree/main/packages/drizzle#multiple-databases) for the full setup, including `mysql2`'s `disableEval: true` and `connectionLimit: 5` Workers defaults.
