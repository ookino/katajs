...
- [x] **MySQL support in `@katajs/drizzle` — `drizzleMysqlAdapter`** — done. Subpath export at `@katajs/drizzle/mysql` (rather than a `dialect` flag on `drizzleAdapter`, which couldn't work cleanly — a top-level `import 'mysql2'` would force every Postgres-only app to bundle mysql2). Each entry point pulls only its own driver; shared bits (Hyperdrive binding lookup) live in an internal `shared.ts`. Built on `drizzle-orm/mysql2` + `mysql2` with Workers-sensible defaults (`connectionLimit: 5`, `disableEval: true` — Workers forbids eval(), and mysql2 ≥ 3 uses it for packet-parser codegen). `mysql2` is an optional peer alongside `postgres`. Slots into the multi-db `db` map alongside the Postgres adapter for mixed-dialect apps. Scaffolder `--db mysql` flag is the remaining nice-to-have (the `posts` example would need a MySQL-schema variant). Shipped 2026-05-11.
- [ ] **Cloudflare D1 support in `@katajs/drizzle` — `drizzleD1Adapter`** — D1 is the SQLite dialect, accessed via a *binding* (`env.DB`) that *is* the client, not a Hyperdrive connection string, and needs no `nodejs_compat`. So it's not a `dialect` flag on `drizzleAdapter`; it's a separate factory exported alongside it — `drizzleD1Adapter({ binding: 'DB', schema })` — that returns `drizzle(env[binding], { schema })` from `drizzle-orm/d1` directly, no pool. Parallels the existing `drizzleAdapter` (Hyperdrive/Postgres) name. `withTransaction` works the same (`db.transaction(fn)`), and it slots into the multi-db map as another named entry: `db: { main: drizzleAdapter({ schema }), cache: drizzleD1Adapter({ binding: 'CACHE', schema: cacheSchema }) }`. Scaffolder gains a `--db d1` option; the `posts` example would need a SQLite-schema variant (`drizzle-orm/sqlite-core` instead of `pg-core`). Keep it in `@katajs/drizzle` — it's still "the Drizzle adapter", just for a different Cloudflare DB product; the `drizzle-orm/d1` import is tiny.
- [x] **Multi-database support** — done. `createApp({ db })` takes a single adapter (unchanged — `c.db` is the client) *or* a named map (`c.db.main`, `c.db.sessions`). `withTransaction` overloaded: `withTransaction(fn)` single, `withTransaction(name, fn)` multi; sub-container rebuild scoped to the named db. `AppDb` augmented to the client type (single) or a map of client types (multi). `katajs add database <name>` does the single→map conversion (config + `AppDb` augmentation + the `c.db` → `c.db.main` rewrite across modules, with anchors for subsequent additions). New `docs/concepts/databases.md`. Shipped 2026-05-11. (The D1 `drizzleD1Adapter` item above slots cleanly into the map as another named entry.)
