...
- [ ] **Switch the Postgres driver from `pg` to postgres.js** — `@katajs/drizzle` currently uses `pg` (node-postgres) + `drizzle-orm/node-postgres` and builds a `pg.Pool`. postgres.js (the `postgres` package) is leaner, has a cleaner API, and is one of Cloudflare's two blessed Hyperdrive + Drizzle paths via `drizzle-orm/postgres-js`. Note: it does *not* remove the `nodejs_compat` requirement — any TCP-based PG client on Workers needs `node:net`/`node:tls`. Scope: rewrite `adapter.ts` (`pg.Pool` → `postgres(connectionString)`), swap the drizzle entrypoint, change the `pg` + `@types/pg` peer/dev deps to `postgres`, update the scaffolder template's `pg` → `postgres`, and verify `withTransaction` still threads (Drizzle's `db.transaction(fn)` abstracts postgres.js's `sql.begin()` — should be fine, but it's the bit to test). Worth doing **before v0.1.0 publishes** so the released adapter starts on the lighter driver rather than swapping it later as a breaking change. ~1 hour. Coordinate with the MySQL item below — if both land, the `dialect` branch picks `postgres` (postgres.js) vs `mysql2`.
- [ ] **MySQL support in `@katajs/drizzle`** — Hyperdrive supports MySQL as well as Postgres, but the adapter is currently Postgres-only (hardcoded `pg` + `drizzle-orm/node-postgres`, builds a `pg.Pool` from `env.HYPERDRIVE.connectionString`). Add a `dialect: 'postgres' | 'mysql'` option that branches the driver (`postgres`/`pg` ↔ `mysql2`) and the matching `drizzle-orm` entrypoint. The `withTransaction` sub-container wiring is dialect-agnostic (`db.transaction(fn)` is the same shape for both), so the divergence is just the ~15-line driver instantiation — a `dialect` option beats a second `@katajs/drizzle-mysql` package that'd be mostly copy-paste. The PG and MySQL drivers become *optional* peers so a Postgres user never installs `mysql2` and vice versa. Scaffolder may want a `--db mysql` flag down the line. Decide before this lands: does the CLI's `posts` example template stay Postgres-only, or do we ship a MySQL variant?
